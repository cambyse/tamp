...
DecisionRule pick-up { # can be dead-end if picked up without identification!
  X, Y, Z, T, U
  { (block X) (location Y) (side T) (on_table X Y) (flipped Z) (colored T) (identified X) (is_final_action U) (hand_empty) }
  { (on_table X Y)! (holding X) (hand_empty)! komoPickUp(X Y Z T U)=1. }
}

DecisionRule pick-up { # can be dead-end if picked up without identification!
  X, Y, Z, T, U
  { (block X) (location Y) (side T) (on_table X Y) (flipped Z) (colored T) (needs_flipping) (is_final_action U) (hand_empty) }
  { (on_table X Y)! (holding X) (hand_empty)! (object_put_down)! komoPickUp(X Y Z T U)=1. }
}

# Put-down
DecisionRule put-down { 
  X, Y, Z, U
  { (block X) (location Y) (side Z) (holding X) (identified X) (needs_flipping)! (colored Z) (is_final_action U) }
  { (holding X)! (hand_empty) (on_table X Y) (orientation_correct) (object_put_down) komoPutDown(X Y Z U)=1. }
}

# Put-down-flipped
DecisionRule put-down-flipped {
  X, Y, Z, U
  { (block X) (location Y) (side Z) (holding X) (flipped FALSE) (needs_flipping) (colored Z) (is_final_action U)}
  { (holding X)! (hand_empty) (bottom_facing side_5)! (bottom_facing side_2) (flipped TRUE) (flipped FALSE)! (needs_flipping)! (orientation_correct) (object_put_down) (on_table X Y) komoPutDown(X Y Z U)=1. }
}
