...
//  if(activateObjectives)  W(komo).addObjective(all, tb, new TM_qLimits(), OT_sos, NoArr, 1.0, 0);
  //
...
//  if(final_action)
//  {
//    mp::Interval final_rejoin{{it.time.to - 0.1, it.time.to}, it.edge};
//    W(komo).addObjective(final_rejoin, tb, new mp::AgentKinEquality( 0, kFrankaCheckConfiguration, kFrankaCheckConfigurationMask ), OT_sos, NoArr, 0.5e2, 0 );
//  }
