...
void groundTreeCheck(const mp::Interval& it, const mp::TreeBuilder& tb, const std::vector<std::string>& facts, KOMO_ext* komo, int verbose)
{
  ...
  if(activateObjectives) W(komo).addObjective(end, tb, new SensorAlignsWithPivot( "franka_hand", facts[0].c_str(), ARR( 0.0, 0.05, 0.0 ), 45.0 * 3.1415 / 180.0 ), OT_ineq, NoArr, 1e2, 0 );
